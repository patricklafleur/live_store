defmodule LiveStoreTest do
  ...
  describe "dispatch/2" do
    test "requires an initialized store" do
      assert {:error, :store_is_misconfigured} = LiveStore.dispatch(nil, {"", %{}}, %{})
    end
  end
  ...
  describe "dispatch/2 with IncrementModuleReducer" do
    test "handle(inc) increments by one by default" do
      assert {:ok, %{counter: 2}} =
               ...
               |> LiveStore.dispatch({"inc", %{}}, %{counter: 1})
    end

    test "handle(inc) increments with the provided value" do
      assert {:ok, %{counter: 5}} =
               LiveStore.new(inc_module_reducer(), sync_dispatcher())
               |> LiveStore.dispatch({"inc", %{v: 4}}, %{counter: 1})
    end
  end
  ...
  describe "dispatch/2 with negating middleware" do
    test "negates the value sent to the reducer" do
      ...
      IO.inspect(store)
      {:ok, %{counter: 4}} = store |> LiveStore.dispatch({"inc", %{v: 1}}, %{counter: 5})
    end
  end
  ...
  describe "dispatch/2 with halting middleware" do
    test "does not call the next function" do
      {:ok, %{counter: 5}} =
        ...
        |> LiveStore.dispatch({"inc", %{v: 1}}, %{counter: 5})
    end
  end
  ...
  describe "store with middleware dispatching" do
    test "handles action synchronously" do
      {:ok, %{counter: 15}} =
        ...
        |> LiveStore.dispatch({"dec", %{v: 10}}, %{counter: 0})
    end
  ...
  defp async_dispatcher(execution_callback) do
    fn store, {action_name, _} = action, state ->
      # Logger.debug "Calling LiveStore.dispatch(#{inspect(action_name)}) asynchronously..."
      task =
        Task.async(fn ->
          ...
          execution_callback.(store |> LiveStore.dispatch(action, state))
        end)
  ...
  defp sync_dispatcher() do
    fn store, action, state ->
      store |> LiveStore.dispatch(action, state)
    end
  end

  defp dispatch_inc_middleware() do
    fn [root_reducer: root_reducer, dispatcher: dispatcher] = store ->
      fn next ->
        fn {action_id, params} = action, state ->
          # process the original "dec" action
          state = next.(action, state)

          case action_id == "dec" do
            true ->
              {:ok, state} = store |> dispatcher.({"inc", %{v: 25}}, state)
              state
  ...
  defmodule IncrementReducer do
    def handle({"dec", %{v: v}} = action, state), do: state |> add(-v)
    def handle({"dec", _} = action, state), do: state |> add(-1)

    def handle({"inc", %{v: v}} = action, state), do: state |> add(v)
    def handle({"inc", _}, state), do: state |> add(1)
